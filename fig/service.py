    ...
    for chunk in output:
        ...
        if 'progress' in event or 'progressDetail' in event:
            ...
            if is_terminal:
                ...
                write(stream, "%c[%dA" % (27, diff))

        print_output_event(event, stream, is_terminal)

        if 'id' in event and is_terminal:
            # move cursor back down
            write(stream, "%c[%dB" % (27, diff))
...
def print_output_event(event, stream, is_terminal):
    ...
    if 'time' in event:
        write(stream, "[%s] " % event['time'])

    if 'id' in event:
        write(stream, "%s: " % event['id'])

    if 'from' in event:
        write(stream, "(from %s) " % event['from'])

    status = event.get('status', '')

    if 'progress' in event:
        write(stream, "%s %s%s" % (status, event['progress'], terminator))
    elif 'progressDetail' in event:
        detail = event['progressDetail']
        if 'current' in detail:
            percentage = float(detail['current']) / float(detail['total']) * 100
            write(stream, '%s (%.1f%%)%s' % (status, percentage, terminator))
        else:
            write(stream, '%s%s' % (status, terminator))
    elif 'stream' in event:
        write(stream, "%s%s" % (event['stream'], terminator))
    else:
        write(stream, "%s%s\n" % (status, terminator))
